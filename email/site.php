...
class Site extends IController
{
	...
	function sendMail(){
     ...
     /* $sql="insert into user2(name,password,email) values('$name','$password','$email')";
   */   
  $data=[
     'name'=>$name,
     'password'=>$password,
     'email'=>$email
      ...
      $user2->setData($data);
      if($id=$user2->add()){
      	$id_new =base64_encode(urlencode($id));
             $t=time();
            $mail= new SendMail();
            $title="激活账户";
            $url="<a href='http://www.iwebshop.com/index.php?controller=site&action=mailJi&id=$id_new&t=$t'>http://www.iwebshop.com/index.php?controller=site&action=mailJi&id=$id_new&t=$t</a>";
           
            $content="邮箱已发送，请注意查收，点击下面链接激活账户$url";
	...
	function mailJi(){
		$id_new=IReq::get('id');
         $id=base64_decode(urldecode($id_new));
		$t=IReq::get('t');
		$data=[
		  'status'=>1,
		];
        if(time()-$t>600){
        	die("链接已超时！！！");
		     ...
		     if($user2->update("id=$id")){
		     	echo "激活成功！！<a href='http://www.iwebshop.com/index.php?controller=site&action=mail_reg'>前往登陆</a>";
        ...
        public function mail_reg_ok(){
        	 $name = IReq::get('name');
             $password = md5(IReq::get('password'));
             $email = IReq::get('email');
              $user2 = new IModel('user2');
             $data= $user2->getObj("name='$name' && password='$password'&& status=1");
            if($data){
            	echo "登陆成功";
            }else{
            	echo "登录失败";
	...
	function index()
	{
		if(isset($_GET['code'])){
		   ...
		   $url="https://graph.qq.com/oauth2.0/me?access_token=$token";
		     ...
		     $left=strpos($str,'(');
		    $right=strpos($str,')');
		     $data=substr($str,$left+1,$right-$left-1);
		...
		//$ip="118.25.91.205"; //上海ip
		$ip="47.94.194.205";   //北京ip
		//$ip=$SERVER['REMOTE_ADDR'];
        $ak='oQTMTPdR300DFZtyeUiP8A0HQK8TQx66';
